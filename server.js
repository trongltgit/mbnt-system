        ...
        try {
            ...
            if (accepted) {
                ...
                await pool.query(`
                    ...
                    (trans_id, pnv_id, pnv_name, trans_date, effective_date, cif, customer_name,
...
app.get('/api/transactions', async (req, res) => {
    try {
        const { pnvId, from, to, status } = req.query;
...
app.put('/api/transactions/:transId', async (req, res) => {
    try {
        const { cif, customerName, effectiveDate, buyCurr, sellCurr, amount, rate, balancedRate, type, editedBy, editNote } = req.body;
        
        await pool.query(`
            UPDATE transactions 
            SET cif = $1, customer_name = $2, effective_date = $3, 
                buy_curr = $4, sell_curr = $5, amount = $6, rate = $7, 
                balanced_rate = $8, trans_type = $9, edited_by = $10, 
                edit_time = NOW(), edit_note = $11, status = 'Đã sửa'
            WHERE trans_id = $12
        `, [cif, customerName, effectiveDate, buyCurr, sellCurr, amount, rate, balancedRate, type, editedBy, editNote, req.params.transId]);
